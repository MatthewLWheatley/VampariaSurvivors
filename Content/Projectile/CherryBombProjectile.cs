...
namespace VampariaSurvivors.Content.Projectile
{
    public class CherryBombControllerProjectile : ModProjectile
    {
        ...
        {
            // Check if any CherryBomb weapon is in the player's inventory
            for (int i = 0; i < player.inventory.Length; i++)
            {
                if (player.inventory[i].ModItem is VSWeapon weapon && weapon.WeaponIdentifier == "CherryBomb")
                {
                    return true;
                }
            }
        ...
        public override void AI()
        {
            ...
            // Check if the CherryBomb weapon is still equipped/in inventory
