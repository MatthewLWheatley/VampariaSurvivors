...
namespace VampariaSurvivors.Content.Projectile
{
    public class MagicWandControllerProjectile : ModProjectile
    {
        ...
        {
            // Check if any MagicWand weapon is in the player's inventory
            for (int i = 0; i < player.inventory.Length; i++)
            {
                if (player.inventory[i].ModItem is VSWeapon weapon && weapon.WeaponIdentifier == "MagicWand")
                {
                    return true;
                }
            }
        ...
        public override void AI()
        {
            ...
            // Check if the MagicWand weapon is still equipped/in inventory
