...
{
    public class PhieraDerTuphelloControllerProjectile : ModProjectile
    {
        ...
        {
            // Check if any PhieraDerTuphello weapon is in the player's inventory
            for (int i = 0; i < player.inventory.Length; i++)
            {
                if (player.inventory[i].ModItem is VSWeapon weapon && weapon.WeaponIdentifier == "PhieraDerTuphello")
                {
                    return true;
                }
            }
        ...
        public override void AI()
        {
            ...
            // Check if the PhieraDerTuphello weapon is still equipped/in inventory
        ...
        private void ShootBullets(Player player)
        {
            ...
            for (int dir = 0; dir < directions.Length; dir++)
            {
                for (int bullet = 0; bullet < weaponStats.Amount; bullet++)
                {
                    ...
                    int projectileType = ModContent.ProjectileType<PhieraDerTuphelloBulletProjectile>();
    ...
    public class PhieraDerTuphelloBulletProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            // Create red energy trail
            if (Main.rand.NextBool(3))
            {
                Dust trail = Dust.NewDustDirect(
                    Projectile.position,
                    Projectile.width, Projectile.height,
                    Terraria.ID.DustID.RedTorch,
                    0, 0, 100,
                    Color.Red,
                    1.2f
                );
