...
namespace VampariaSurvivors.Content.Projectile
{
    public class FireWandControllerProjectile : ModProjectile
    {
        ...
        {
            // Check if any FireWand weapon is in the player's inventory
            for (int i = 0; i < player.inventory.Length; i++)
            {
                if (player.inventory[i].ModItem is VSWeapon weapon && weapon.WeaponIdentifier == "FireWand")
                {
                    return true;
                }
            }
        ...
        public override void AI()
        {
            ...
            // Check if the FireWand weapon is still equipped/in inventory
