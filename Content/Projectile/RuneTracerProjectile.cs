...
namespace VampariaSurvivors.Content.Projectile
{
    public class RuneTracerControllerProjectile : ModProjectile
    {
        ...
        {
            // Check if any RuneTracer weapon is in the player's inventory
            for (int i = 0; i < player.inventory.Length; i++)
            {
                if (player.inventory[i].ModItem is VSWeapon weapon && weapon.WeaponIdentifier == "RuneTracer")
                {
                    return true;
                }
            }
        ...
        public override void AI()
        {
            ...
            // Check if the RuneTracer weapon is still equipped/in inventory
