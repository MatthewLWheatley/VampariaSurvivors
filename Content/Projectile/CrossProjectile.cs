...
namespace VampariaSurvivors.Content.Projectile
{
    public class CrossControllerProjectile : ModProjectile
    {
        ...
        {
            // Check if any Cross weapon is in the player's inventory
            for (int i = 0; i < player.inventory.Length; i++)
            {
                if (player.inventory[i].ModItem is VSWeapon weapon && weapon.WeaponIdentifier == "Cross")
                {
                    return true;
                }
            }
        ...
        public override void AI()
        {
            ...
            // Check if the Cross weapon is still equipped/in inventory
