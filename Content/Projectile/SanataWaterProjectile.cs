...
namespace VampariaSurvivors.Content.Projectile
{
    public class SantaWaterControllerProjectile : ModProjectile
    {
        ...
        {
            // Check if any SantaWater weapon is in the player's inventory
            for (int i = 0; i < player.inventory.Length; i++)
            {
                if (player.inventory[i].ModItem is VSWeapon weapon && weapon.WeaponIdentifier == "SantaWater")
                {
                    return true;
                }
            }
            ...
            for (int i = 0; i < player.inventory.Length; i++)
            {
                if (player.inventory[i].ModItem is VSWeapon weapon && weapon.WeaponIdentifier == "SantaWater")
                {
                    ...
                    return currentStats.Damage != weaponStats.Damage ||
                           ...
                           currentStats.Duration != weaponStats.Duration ||
        ...
        public override void AI()
        {
            ...
            // Check if the SantaWater weapon is still equipped/in inventory
