...
namespace VampariaSurvivors.Content.Projectile
{
    public class KingBibleControllerProjectile : ModProjectile
    {
        ...
        {
            // Check if any KingBible weapon is in the player's inventory
            for (int i = 0; i < player.inventory.Length; i++)
            {
                if (player.inventory[i].ModItem is VSWeapon weapon && weapon.WeaponIdentifier == "KingBible")
                {
                    return true;
                }
            }
        ...
        public override void AI()
        {
            ...
            // Check if the KingBible weapon is still equipped/in inventory
